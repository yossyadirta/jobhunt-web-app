...
import { editJob } from "../store/actions/jobsAction";
import Loading from "../components/Loading";
import Swal from "sweetalert2";

export default function FormEditJob({ detailJob, skills }) {
  ...
  const [input, setInput] = useState({
    ...
    companyId: detailJob.companyId,
    jobType: detailJob.jobType,
  });
  ...
  const [inputSkill, setInputSkill] = useState(skills);
  ...
  const handleSubmit = (e) => {
    ...
    if (!input.title) {
      ...
      dispatch(editJob(detailJob.id, input, inputSkill)).then((res) => {
