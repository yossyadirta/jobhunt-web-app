...
import { editCompany } from "../store/actions/companiesAction";
import Swal from "sweetalert2";

export default function FormEditCompany({ detailCompany }) {
  const dispatch = useDispatch();
  const navigate = useNavigate();

  const [input, setInput] = useState({
    name: detailCompany.name,
    companyLogo: detailCompany.companyLogo,
    location: detailCompany.location,
    email: detailCompany.email,
    description: detailCompany.description,
  });
  ...
  const handleSubmit = (e) => {
    ...
    if (!input.name) {
      ...
      dispatch(editCompany(detailCompany.id, input))
        .then(() => {
          navigate("/companies");
        })
        .catch((err) => {
